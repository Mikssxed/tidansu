...
1. Read `.pi/context/frontend-rules.md` (not auto-loaded). The
...
3. Look at neighbours in `src/components/base/`, `form/`, `layout/`, and the
   ...
   `.pi/skills/create-frontend-component.md` and
   `.pi/templates/vue-component.vue` / `frontend-view.vue`.
...
- **Variant-based styling.** A `variant` (and optional `size`) union prop mapped
  to **complete static class strings** via `Record`. Never pass raw class strings
  or hex through props.
