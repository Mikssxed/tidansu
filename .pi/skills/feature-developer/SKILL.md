...
2. `CLAUDE.md` is already in your context (project instructions) and its key rules
   ...
   `.pi/context/*.md` rules for the layer you're touching (those are *not*
   auto-loaded). Follow the matching `.pi/skills/*.md` walkthrough and
   `.pi/templates/*` shape (e.g. `create-cqrs-command.md` + `cqrs-*.cs`,
...
**Frontend.** `<script setup lang="ts">`. Variant union prop → `Record` static class
map; **`@theme` token colors only**, never hex; **static Tailwind classes** (no
`` `bg-${x}` ``); **`computed` for every derived value/class**; **named handlers
only**. Honour the **template-purity HARD RULE** — zero logic in `<template>`; for
a `v-for` needing derived values, build a fully-mapped computed array and iterate
that. Never use `any`; never duplicate a Kiota type (use the generated model);
narrow nullable Kiota fields with a typed computed. Data access via a TanStack
Query composable; mutations invalidate query keys and, on a 403 `{plan:[reason]}`,
open the paywall.
