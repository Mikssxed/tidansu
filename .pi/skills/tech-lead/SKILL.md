...
3. `.pi/context/architecture.md`, `backend-rules.md`, `frontend-rules.md` —
...
Ignore `.pi/context/project-overview.md` (stale SelfGrind task/XP text). Treat
`.pi/skills/*.md` and `.pi/templates/*` as the canonical shapes — a new CQRS command
follows `create-cqrs-command.md` + `templates/cqrs-*.cs`; a new Vue component follows
`create-frontend-component.md` + `templates/vue-component.vue`.
