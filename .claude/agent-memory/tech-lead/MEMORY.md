...
- [Server over-cap read-only gate](arch_server-overcap-readonly-gate.md) — B-24: whole-space over-cap gate is a DISTINCT boundary from per-space count caps; shared SpaceOverCapGuard; rank query must be collation-matched to OrderBy(Id) or SPA/server disagree
</content>
