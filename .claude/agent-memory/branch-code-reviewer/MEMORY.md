...
- [CI drift-check CRLF false positive](ci-drift-check-crlf-false-positive.md) — regen+git-diff gate on a Linux runner reds a clean artifact when repo has no .gitattributes and blobs go CRLF
- [Unique-index dedupe without a catch](unique-index-dedupe-no-catch.md) — check-then-insert backed by a unique index 500s under a race unless the insert catches DbUpdateException
